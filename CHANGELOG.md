...
Keyboard-first, on `127.0.0.1:7654`. `←`/`→` scrub the timeline, `c` and `d`
switch the inspector to the context view and the context diff, `[`/`]` move the
diff baseline, `t` opens the fork tree, `o` the runs overlay, `y` copies the
equivalent `tape …` command, `?` lists the rest. `tape ui <run>` opens on that
run; bare `tape ui` opens the newest one with the overlay up, because a
debugger should open on the thing being debugged.

### What it shows, and why it is only these things

Six screens, chosen against one test: *could a general observability tool
render this from its own data?* If yes, it is out. LangSmith and Braintrust
ship polished UIs backed by teams; competing on breadth loses on every axis at
once. What is left is the surface nobody else has, because nobody else records
it:

- **The context view and context diff**, with truncation called out. A message
  that survived but lost most of itself gets its own gutter, its kept prefix
  and its dropped tail as separate blocks — an inline diff of "the last 511
  characters vanished" is unreadable, and this is the bug the demo is built
  around.
...
- **The chain tree** for LangChain and LangGraph runs, with the HTTP and LLM
  events nested inside the node they happened in — the relationship the
  transport layer alone cannot see.
- **Doctor findings grouped by call site**, recomputed from stored runs so a
  finding is never stale.

There are no cost charts, no latency percentiles, and no cross-run aggregates.
That is the scope, not an omission.

### Three constraints, which are the reasons to trust it

- **Local only.** It binds `127.0.0.1`, and that is not a flag. No accounts, no
  auth, no telemetry, and the page loads nothing remote — so it works with no
  network at all. Redaction is best-effort pattern-matching, so the honest
  assumption is that a trace may still hold something private; "unreachable
  from the network" is what makes "no auth" correct rather than careless.
- **No dependencies.** `reeltime` still installs with `dependencies = []`. The
  server is `http.server` from the standard library and the frontend is one
  HTML file with inlined CSS and JS: no build step, no CDN, nothing to audit.
  A debugger that needs a toolchain to look at a crash is a debugger you will
  not open.
- **It never runs your agent.** No fork button and no run-doctor button.
  Forking spawns your code with live credentials and real cost, and a viewer is
  exactly where someone clicks by accident. The UI makes forks legible — the
  tree shows lineage and every screen hands you the `tape fork` command — and
  leaves execution in the shell, where the output is visible.

Also read-only in the strict sense: no route mutates a trace, so there is no
write path to audit. There is a test asserting the tape directory's mtimes are
unchanged after every endpoint has been hit.

### Added

- **Location-hash deep links.** The URL always describes what is on screen
  (`/run/<id>#diff/1/0`), so a view is linkable — and it is what lets the
  README screenshot be regenerated by driving a headless browser to a URL,
  with no browser-automation dependency.
- **A viewer screenshot in the README**, regenerable with
  `./tools/capture_ui.sh`. Separate from `vhs demo.tape` because vhs records a
  terminal and this is a browser page.

### Changed

- The CLI help no longer prints a `planned:` epilog. Every verb this project
  ever promised now exists, and a label with nothing after it is worse than no
  label.
- The status bar reads `event 1 · 2 events` rather than `event 1 of 1`, which
  described a two-event run as though it had one.
- The run header and call site show a basename rather than an absolute path,
...
- **The viewer cannot drift from the CLI.** Every endpoint calls the same
  ...
  serialises the result; it computes nothing of its own. That is what makes
  "no new event data, no recording changes" hold by construction, and there
  are tests asserting each payload equals what those functions returned.
